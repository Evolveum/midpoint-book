...
That makes directory servers almost ideal candidates for shared account database.
...
Sadly, there was no major update to LDAP specifications in decades.
...
However, you should be aware this a temporary solution.
It has many limitations.
The right way to do it is to use _authentication server_ instead of directory server.
...
There are mechanisms to _identity over-privileged users_ by analysing risk scores of individual users.
...
// TODO: Workflow orchestration: useless for approvals, but may be useful for enrollment, remediation, etc. Organization-wide workflow platform would be preferred over IDM-specific platform.
...
=== Identity Governance Terminology

// TODO: terminology note
// This field (or sub-field) is quite diverse, therefore the terminology and even the concepts may seem somehow chaotic.
...
Simply speaking, we are going to treat corporate network in exactly the same way as we are treating public Internet.
...
Cloud applications need to wort together with on-premise applications.
...
Therefore the zero trust approach is often combined with introduction of new authentication mechsnisms.
...
// TODO: what to do in identity for zero trust?
// There are IDM system, but not many are well-prepared.
// There was not much demand yet. People do not realize full scale of the problem.
// Make sure your IDM system can evolve to support this.
