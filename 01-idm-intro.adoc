...
False denial of role assignment is likely to trigger an immediate and often quite emotional feedback from the requester.
...
However, the artificial intelligence is only as good as the training data.
...
// TODO: Identity analytics/intelligence
// Risk-based approach, holistic, integrates several mechanisms
// Role mining
// Outlier detection
// Identifying over-privileged users (risk)
// Detection of high-privilege entitlement to a low-privilege user
// Can be integrated with AM, e.g. require stronger authentication for high-risk users
