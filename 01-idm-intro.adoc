...
The security requirements dictate the need for authentication and authorization of users.
Authentication is a mechanism by which the computer checks that a user is really who they pretend to be.
Authorization is a related mechanism by which the computer determines whether to allow or deny the user to perform a specific action.
...
A user presents an identifier and a password.
...
That's where the real story of digital identity begins.

.Enterprise Identity and Access Management
NOTE: This book deals mostly with _Enterprise Identity and Access Management_.
...
However, many of the mechanisms and principles described in this book can also be applied to non-enterprise environments.


== Directory Services and Other User Databases

The central concept of identity management is a data record which contains information about a person.
This concept has many names: _user profile_, _persona_, _user record_, _digital identity_ and many more.
The most common name in the context of identity management is _user account_.
Accounts usually hold information that describes the real-world person, represented by attributes such as given name and family name.
However, probably the most important part of the _account_ is the technical information that relates to the operation of an information system for which the account is created.
This includes operational parameters such as location of the user's home directory, wide variety of permission information such as group and role membership, system resource limits and so on.
...
Regardless of the specific method used to store and process the records, the _account_ is undoubtedly one of the most important concepts of IAM field - and so are the databases where the _accounts_ are stored.
...
_Directory servers_ are built with the primary purpose to provide shared data storage to applications.
...
A shared identity store is making user management easier.
An account needs to be created and managed only in one place.
Authentication still happens in each application separately.
Yet, as applications use the same credentials from a shared store, users may use the same passwords for all the connected applications.
...
LDAP would deserve a major review, to correct the problems and bring the protocol to the 21^st^ century.
...
LDAP still remains a major protocol in the Identity and Access Management field.
...
Nothing more.
The protocols and interfaces (APIs) that are used to access directory servers are designed as database interfaces.
They are good for _storing, searching and retrieving_ data.
While the _user account_ data often contain entitlement information (permissions, groups, roles, etc.), directory servers are not well-suited to _evaluate_ them.
This means that a directory server can provide information about what permissions an account has, but it is not designed to make a _decision_ whether to allow or deny a specific operation.
Also, directory servers do not contain data about user _sessions_.
...
Yet, directory servers were not designed to do this.
...
Being databases, they should be used as such.
...
Nevertheless, you should be aware that this is a temporary solution.
It has many limitations.
The right way to do it is to use an _authentication server_ instead of a directory server.
Access Management (AM) technologies can provide that.

=== Single Directory Server Myth

Now listen, this is a nice and simple idea: Let's keep all our user data in a single directory server.
...
Here, all identity management problems are solved!

Unfortunately, they are not.
A shared directory server makes user management easier.
...
However, the HR system is usually not authoritative for assignments of employee identifiers such as _username_.
...
For example, a management information system may be responsible for determining user roles (e.g., in a project-oriented organizational structure).
An inventory management system may be responsible for assigning telephone numbers to users.
A groupware system may be an authoritative source of user e-mail addresses and other electronic contact data.
There are usually 2 to 20 systems that provide authoritative information for a single user.
Therefore, there is no simple way to feed and maintain the data in the directory system.

Then there are spacial and technological barriers.
Many complex applications need local user databases.
They must store copies of user records in their own databases to operate efficiently.
For example, large billing systems cannot work efficiently with external data (e.g., because of a need to make a relational database _join_).
Therefore, even if a directory server is deployed, these applications still need to maintain a local copy of identity data.
Keeping the copy synchronized with the directory data may seem like a simple task.
But it is not.
Additionally, there are legacy systems which usually cannot access the external data at all (e.g., they do not support the LDAP protocol at all).
...
While the account creation can be usually done in an on-demand fashion (e.g., create a user directory at first user log-on), the modification and deletion of accounts is much more difficult.
Directory servers will not do that.
...
If every application has its own set of attributes to control access control policies, then a centralized directory provides only a negligible advantage.
The attributes may as well reside in the applications themselves.
That's exactly how most directory deployments end up.
Directory servers contain only the groups, groups that usually roughly approximate low-level RBAC roles.

Quite surprisingly, LDAP standards themselves create a significant obstacle to interoperability in this case.
...
Therefore, access control policies and fine-grained authorizations are usually not centralized in directory servers, they are maintained directly in the application databases.

A _single directory approach_ is feasible only in very simple environments or in almost entirely homogeneous environments.
...
Access management (AM) systems handle all the flavors of authentication, and even some authorization aspects.
The principle of all access management systems is basically the same:

. The access management system gets between the user and the target application.
This can be done by a variety of mechanisms, the most common method is that the applications themselves redirect the user to the AM system if they do not have an existing session.

. The access management system prompts users for the username and password, interacts with users' mobile devices, or otherwise initiates the authentication procedure.

. Users enter credentials.

. The access management system checks the validity of credentials and evaluates access policies.

. If access is allowed, the AM system redirects users back to the application.
...
. Application validates the token, creates a local session and allows access.
...
All they need to know is that the user _was_ authenticated, and that the authentication was strong enough.
...
Single Sign-On (SSO) systems allow users to authenticate once, and access a number of different systems re-using that authentication.
...
The general access management flow is described below:
...
. The access management server establishes a session (SSO session) with the user browser.
This is a crucial part of the SSO mechanism.

. The user is redirected back to application `A`.
Application `A` usually establishes a local session with the user.

. The user interacts with application `A`.

. When the user tries to access application `B`, it redirects them to the access management server.

. The access management server checks for the existence of an SSO session.
As the user authenticated with the access management server before, there is a valid SSO session.

. The access management server does not need to authenticate the user again.
It immediately redirects the user back to application `B`.

. Application `B` establishes a local session with the user and proceeds normally.

The user does not usually even realize that there were any redirects when accessing application `B`.
There is no interaction between the redirects, and the processing on the access management server is usually very fast.
It looks like the user was logged into application `B` all the time.
...
The request of a user accessing an application is directly or indirectly passed through an access management server.
...
The AM server usually intercepts only the first request to access the application, because there would be a performance impact if all requests were intercepted.
...
This means it can only enforce a very _coarse-grained_ authorization decisions.
In practice, this usually means that the AM server can make only all-or-nothing authorization decisions: whether a particular user can access all parts of a particular application, or whether they cannot access the application at all.
The AM server usually cannot make any fine-grained decisions just by itself.

Some AM systems provide agents that can be deployed to applications, agents that enforce fine-grained authorization decisions.
Such agents often rely on HTTP communication.
They are making decisions based on the URLs that the user is accessing.
This approach might have worked well in the 1990s but it has only very limited applicability in the age of single-page web and mobile applications.
...
For example, an e-banking application may allow or deny a transaction based on the sum of previous transactions made earlier that day.
...
It would be a major burden to keep such information updated and consistent, not to mention the security and privacy concerns.
...
In theory, the authorization system should only make allow/deny decisions.
However, this is not enough to implement an efficient application.
The application cannot afford to list all the objects in the database, pass them to an authorization server, and then realize that the authorization server denied access to almost all of them.
Authorization has to be processed _before_ the search operation, and additional search filters have to be applied.
...
The vast majority of the authorization decisions is carried out by each individual application and is completely outside the reach of an AM system.


=== SAML and OpenID Connect

Some access management systems use proprietary protocols to communicate with applications and agents.
This is obviously an interoperability issue – especially when AM principles are used in the Internet environment.
Indeed, it is the Internet that motivated standardization in this field.

The first widespread standardized protocol in this field was the Security Assertion Markup Language (SAML).
...
Overall, SAML is not just a protocol, it is a set of very rich and flexible mechanisms.

The primary purpose of SAML is the transfer of identity information between organizations.
...
XML and SOAP-based web service mechanisms are hopelessly out of fashion, which impacts the popularity of SAML.
...
That probably contributed to the development of the OpenID Connect protocol (OIDC).
OpenID Connect is based on much simpler mechanisms than SAML, but it reuses the same basic principles.
...
That was followed by the development of the OpenID protocol, which was still very simple.
...
At that time, there was an almost unrelated protocol called OAuth, which was designed for management of cross-domain authorizations.
...
The OAuth2 framework was used to build a cross-domain authentication and a user profile protocol.
...
Now there are two protocols that are using the same principle and doing almost the same thing: SAML and OpenID Connect.
The principle of both protocols is illustrated in the following diagram.
...
This can be a web page or web application running on the target site.

. The target site does not have a valid session for the user.
Therefore, it redirects the user browser to the source site.
It adds an authentication _request_ into that redirect.

. The browser follows the redirect to the source site.
The source site gets the authentication _request_ and parses it.

. If the user is not already authenticated with the source site, the authentication happens now.
The source site prompts for the username, password, certificate, one-time password, or another credential required by the policy.
With a bit of luck, the authentication succeeds.

. The source site redirects the browser back to the target site.
The source site adds an authentication _response_ to the redirect.
The most important part of the response is a _token_.
The token directly or indirectly asserts the user's identity.

. The target site parses the authentication _response_ and processes the token.
The token may be just a reference (e.g., a SAML artifact) pointing to real data, or it may be an access key to another service that provides the data (OIDC UserInfo).
In such a case, the target site makes another request (6a).
This request is usually a direct one, and it does not use browser redirects.
One way or another, the target site now has claims about the user identity.

. The target site evaluates the identity, processes authorizations, and so on.
...
The target site finally provides content.

The following table compares the terminology and technologies used in the SAML and OIDC worlds.
...
A attentive reader has surely noticed some similarity with web-based access management mechanisms.
That's right.
This is the same wheel, reinvented over and over again.
However, to be completely honest, we have limited our description to cover only flows intended for web browsers.
Both SAML and OIDC have broader applicability than web browser flows alone.
The differences between the two protocols are much more obvious in these extended use cases.
However, the web browser case nicely illustrates the principles and similarities of SAML, OpenID Connect, and also the simple web-SSO systems.

Maybe the most important differences between SAML, OIDC and web-SSO (also known as _Web Access Management_ or WAM) systems is the intended use:

* SAML was designed for the web applications and SOAP for the web services world.
SAML will handle centralized (single-IDP) scenarios very well, but it can also work in decentralized federations.
Go for SAML if you are using SOAP and WS-Security, or if you plan to build an extensive decentralized federation.
On second thought, you should probably forget about SAML anyway.
It is not very fashionable these days.
...
Technologically, it will fit into the RESTful world much better than SAML.
The current fashion trends are favorable to OIDC.

* Web-SSO/WAM systems are designed to be used inside a single organization.
It is ideal to implement SSO between several customer-facing applications so that customers have no idea that they interact with multiple applications, not just one.
The web-SSO/WAM systems are not designed to work across organizational boundaries.
This is quite unfortunate in the current "cloudy" IT environment.
...
However, it has to be expected that an SSO system based on SAML or OIDC will have slightly more complicated setup than a simple Web-SSO/WAM system.
...
The classic classroom example of non-web SSO system is _Kerberos_.
...
Many network devices use _RADIUS_ protocol for what network engineers call "Authentication, Authorization and Accounting" (AAA).
...
However, there are usually no solutions for deleting the records or for updating the records of inactive users.
...
When it is implemented in the AM server, it is re-used by all the applications without additional investment.
...
Additionally, the redirection approach of AM systems assumes that the user has something that can display authentication prompts and carry out user interaction.
...
Each island needs to be managed.
...
In IAM program, it is absolutely critical for success.
...
IDM does many other things that are absolutely essential for every organization to operate in an efficient and secure manner.

It looks like IDM is the best thing since the sliced bread.
So where's the catch?
Oh yes, there is a catch.
At least, there _was_ a catch.
...
However, the benefits clearly outweigh the costs now.
...
The integration problems motivated the inception of identity management technologies in early 2000s.
...
Most of them do not exist today, or are considered to be legacy.

All these IDM systems were commercial closed-source software.
...
Even worse, the closed-source software is prone to vendor lock-in.
...
New products appeared since early 2010s, in many areas in identity and access management.
However, it is still a humble identity management platform that forms the core of the solution.
The products have evolved to the point, that the entire field is now called _identity governance and administration_ (IGA).
...
_Identity management_ (IDM) is a simple term which encompasses a very rich and comprehensive functionality.
...
The IDM policies say that the code `S007` means sales agent and that anybody with that code should automatically receive `Sales Agent` role.
...
Something unexpected happens the following month.
...
Later, the investigation reveals that Oscar is mostly innocent.
...
The IDM system is used to permanently disable all Mallory's accounts, re-enable Oscar's accounts, and also to revoke powerful privileges that are considered too risky for Oscar to have.
...
The security officers set up automated _access certification_ campaign.
...
The manager must confirm (_certify_) that the user still needs those privileges.
...
The basic principle is very simple: identity management system is just a sophisticated data synchronization engine at its core.
...
It is all about the details - the way how the IDM system gathers the data, how it is processing the data and how it is propagating the changes that make all the difference.

In addition to that, there are interesting possibilities for analysis and governance of identity data.
...
We can also _govern_, distribute maintenance of data to departments, nominate role owners, set up processes to maintain the policies ... we can all sorts of interesting things.

However, it all depends on the _data_.
The _data_ are at the very core of identity management, administration and governance.
We need to have the data, taken from reliable source, transformed, unified, and most importantly, always kept fresh.
This is the most important responsibility of identity management system.
...
Connectors are usually accessing external interfaces (APIs) of source and target systems.
...
The connector-based _architecture_ is pretty much standard among all advanced IDM systems.
Yet the connector _interfaces_ significantly vary from one IDM system to another.
...
Except for one case.
The _ConnId connector framework_ is the only connector interface that is actively used and developed by several competing IDM systems.
It is perhaps no big surprise that ConnId is an _open source_ framework.

Even though connector-based approach is quite widespread, some (mostly older) IDM systems are not using connectors.
...
However, it is still a major consideration whether it is worth the trouble of maintaining the agents.
...
However, even this feature is still not entirely common.
...
One of the most important features of any self-respecting IDM system is to make sure that everything _is right_, and also that it _stays right_ all the time.
Identity management is all about _continuous_ maintenance of the identities.
Without that continuity the whole IDM system is pretty much useless.
...
Applications do not provide suitable interfaces for real-time synchronization very often.
...
Some systems cannot afford to run it more frequently than once per month.
...
There are a lot of users in our systems, every user needs a specific set of permission to access the applications.
...
Top layers of the hierarchy are usually composed of business roles such as `Marketing specialist`.
...
Role hierarchy is often a must when the number of permissions and users grows.
...
As most of the role management is driven by a policy, we call this approach _policy-driven RBAC_.
...
Therefore, whether you like it or not, RBAC is the primary mechanism of any practical IDM solution - and it is here to stay.
...
take the _subject_ (user), _operation_ and _object_ (the things that user is trying to access).
Evaluate whether the policy allows access for that _subject-operation-object triple_.
...
This principle is rooted very deeply in information security tradition.

However, in the identity management field, we need to think quite differently.
...
Therefore, the concept of authorization is turned completely upside down in the IDM world.
...
Definitely not.
The IDM system does not _enforce_ authorization decisions.
...
However, it indirectly _manages_ the data that are used to evaluate authorizations.
...
Most applications do not care about authentication at all, they just need to know that the user was authenticated somehow, and know the user's identifier.
Applications do not care about authentication details.
Which makes _authentication_ relatively easy to manage.

However, it is quite a different story for _authorization_.
...
There are approaches that partially address some parts of authorization problems.
...
Agents can be very useful for some cases, however they are not universal solution, at least not yet.

Therefore, it is not realistic to expect that the authorization could be completely centralized anytime soon.
The authorization policies still need to be _distributed_ into the applications.
...
Alternatively, the IDM itself may be used to manually maintain an organizational structure.
...
However, it is very likely that we will see more stories about this in the future.
...
While this functionality provides benefits in almost any deployment, organizational structure management is absolutely essential for organizations that are based on tree-like functional organizational structures.
...
And so on.
Good IDM system can do all of that using just a handful of configuration objects.

This seems to be almost too good to be true, which it somehow is.
...
The _identity management_ proper, sometimes called _identity administration_, is focused mainly on technical aspects of identity life-cycle such as automatic provisioning, synchronization, evaluation of the roles and computing attribute values.
...
E.g. a governance system may deal with _segregation of duties_ policy.
It may drive the process of _access certification_.
It may focus on automatic _analysis and reporting_ of the identity, auditing and policy data.
It drives _remediation_ processes to address policy violations.
It manages application of new and changed policies, evaluate how is your system _compliant_ with policies and regulations and so on.
It may evaluate _risk_ levels, modeling overall organizational risk, identifying risk hot-spots.
...
Back in the 2010s, it was a common practice for identity governance features to be implemented by specialized products that are separated from their underlying IDM platforms.
...
It perhaps comes without saying that reasonable IDM/IGA solutions should offer both the IDM and governance features in one unified and well aligned product.
...
Such a manager could nominate a _deputy_ that would receive some of manager’s privileges for a limited time period.
...
Even though the access request process cannot be completely replaced, policies should be applied as broadly as it is possible.

* *Entitlement management* deals with entitlements of user’s accounts in target systems, such as role or group membership.
...
Therefore, there are _access certification campaigns_ that are also known as "certification", "re-certification" or "attestation" mechanisms.
...
On the other hand, there are _micro-certifications_.
These are very small certification actions, usually involving just a single user.
They are triggered automatically, for example as a reaction of re-assigning user to a different organizational unit, or by accumulating dangerous risk level for one user.
+
...
+
Yet again, the best way to handle certification effort is to avoid certification altogether.
Policy-based approach relies on roles assigned automatically.
Which also means they are _unassigned_ automatically, and there is no need for case-by-case certification.
Of course, the policy needs to be created, properly maintained and reviewed.
However, the effort to maintain a policy is likely much smaller than certification effort.
...
Therefore, many role _owners_ are usually nominated to take care of role maintenance.
...
* *Identity analytics and intelligence (IdA)* is mostly an umbrella term.
...
Write down your expectations, select a product, and conduct a _proof-of-concept_ test tuned to your specific needs.
...
As the requirements are so complex and often even contradictory, it is very unlikely that there ever will be any single product that can do it all.

A clever combination of several components is needed to build complete solution.
The right mix of ingredients for this IAM soup will always be slightly different, as no two IAM solutions are exactly the same.
...
Very simple.
Also, the identity store needs to be properly managed.
...
This is usually provided by the _directory service_.
...
It allows system administrators and security officers to live happily, to breath easily and to keep control over the whole solution.
...
Security studies quite consistency rate the _insider threat_ as one of the most severe threats for an organization.
...
Yet, even the worst IDM system can do that.
...
Yet, advanced IDM systems already support automation of this access certification process.
...
Yet, a machine can do that easily and reliably.
...
However, we can _mitigate_ the risk by preparing plans to be executed when we are affected by a zero-day vulnerability.
...
Risk mitigation is focused on making the impact of such an attack less painful, reducing the damage.
...
Then the data can be fed into a risk model, evaluating how are the entitlements combined in roles, how are the roles assigned to users, identifying high-risk roles and users.
...
_Zero-trust_ is an approach to design network and application systems.
...
The old approach was replaced by a new one: _zero trust_.
...
Traditional enterprise integration patterns based on _soft interior_ do not work in this brave new world anymore.
...
The most basic requirement is an _application inventory_, an authoritative list of applications in an organization.
...
//== Digital Identity
//
//Strictly speaking, _digital identity_ is any digital representation of one's identity.
//It is a set of characteristics, qualities, believes and behaviors of en entity, usually represented as a set of attributes.
//Digital identity is a data record, usually structured, intended for machine processing.
//Such identity usually describes a person, however it can describe a non-person entities as well.
//
//That is a dictionary definition of _digital identity_.
//However, this is IAM field, it would be just too easy if one simple definition can fully describe a concept.
//It is perhaps not a big surprise that in practice _digital identity_ is many things to many people.
...
//Idealistically, the user should be able to see and manage all the aspects of her or his digital identity, modify identity attributes, manage the relationships with other entities, control how the identity is presented.
//The user should be able to control how the data are shared with other sites and applications, log into other sites without a need to use passwords, manage consents, initiate data erasure and so on.
//That is the idea.
//However, the reality is far from being ideal, and there are some practical obstacles.
//
//While a person's identity is intangible, the _digital_ identity is composed of data.
//The data need to reside somewhere.

// TODO: centralized identity, social media

// TODO: identifiers, DID

// TODO: who should manage the identity? Transparency? How much is user in control? Who owns the data?
// TODO: the identity is not presented by the user alone, but by other sites and organizations
// TODO: questions transcend to humanities, philosophy and ethics
...
// TODO: Customization on both sides
// TODO: adjust your processes, as well as customize the IDM product

// TODO: cloud, pre-packaged solutions, customization limitations
// * Cloud, SaaS, etc.
//   difficulty for customization
//   autonomy

// TODO: IAM program (not a project)
// IAM is not just the technology
// cloud does not change that, you have to program running even if you have cloud.
