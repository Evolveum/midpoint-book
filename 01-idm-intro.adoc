...
LDAP roots are going as far back as 1980s to a family of telecommunication protocols known as X.500.
Even though LDAP may be old it is far from being obsolete.
Quite the contrary.
...
. Access management system prompts user for the username and password, requests the certificate, creates a challenge and prompts for the response or in any other way initiates the authentication procedure.
...
This approach might work well in the 1990s, but it has only very limited applicability in the age of single-page web applications and mobile applications.
...
And the latest fashion trends are not very favorable to SAML.
XML and SOAP-based web service mechanisms are getting out of fashion.
...
And in fact OAuth2 is not really a protocol at all.
...
And in the vast majority of cases those databases are directory services.
...
The AM systems usually assume that the underlying directory system is already properly managed.
...
The underlying directory service is almost always a hard requirement for even humblest AM functionality.
...
Although all the products are targeting the IDM space they are all significantly different.
...
|2 000 – 20 000
|You really need IDM.
You simply cannot manage that crowd manually.
...
On the other hand, identity governance abstracts from the technical details and it is focused on policies, roles, business rules, processes and data analysis.
...
Such a manager could nominate a deputy that should receive parts of manager’s privileges.
...
This leads to an accumulation of privileges over time.
...
Therefore approval of each individual role change may be too annoying and even harmful.
...
Simple process will ask for yes/no decisions or may ask to nominate users.
...
And this database is indeed widely shared by applications that are capable to connect to it.
...
And especially the LDAP sever topologies can usually scale ad nauseam.
...
It makes sure that the data are up to date and compliant with the policies.
