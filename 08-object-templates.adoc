...
But the things identity management systems do _internally_ are crucial to all identity management deployments - especially those that deal with identity governance and compliance.
...
Inbound mappings cannot do that as they are tied to a particular resource.
...
When everything is set up, you can try to manually import a single account from the HR resource by using the “import” button on the page where you can list resource accounts.
