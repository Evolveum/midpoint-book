...
Identity management systems are often seen as an integration engines that move data from one system to another system.
This is indeed a very important part of the identity management functionality.
But the things that the identity management systems does _internally_ are crucial to all identity management deployments – especially those that deal with identity governance and compliance.
...
There are complex and very powerful mechanisms that allow midPoint to implement identity governance.
But more on that later.
We need to start with simple things.
And the simplest of those internal mechanisms is the functionality of _object template_.
...
Inbound mappings can be used to sort out some of those problems.
But inbound mappings are designed to work in isolation.
They work only for one particular resource.
But it is often needed to gather data from several resources and then look at all of them at once.
As inbound mappings cannot do that as they are tied to a particular resource.
However, inbound mappings can be used to gather all the relevant data in the user object.
And then we can use some kind of a mechanism to have a look at user object when those data are gathered together.
...
For example, a very typical use of object template is computation of user’s full name:
...
Object template can be used to do variety of things to all kind of midPoint objects.
...
The template will not be used just by being imported.There can be several object templates for different types of objects, archetypes and even organizations.
...
But object template can also to other tricks.
...
Therefore almost all of the object template functionality is located in item element that references a particular item by its path:
...
But more on that in a next section.
...
But it is not very usual or intuitive in cultures that are not used to it (which means all the cultures).
...
Similarly for "given name" and "family name".
...
MidPoint insists to have name set for all the users.
But we may be able to compute a name from other properties, such as other user names, employee number or other identifiers. not want to present name item as mandatory in the user interface.
...
But core midPoint engine will still require a name to have a value.
This given object template a chance to generate the value for name.
However, this means that name will still be present as read-write item in the user interface.
We do not want that if name is supposed to be immutable identifier.
...
Therefore privileges users such as system administrator should be able to change the identifiers if really needed.
Therefore the proper way how to do this would be to use authorizations and not object template.
But we do not know how to use authorizations yet.
Therefore this will have to do for now.
...
It makes little sense to present the items that are not used, therefore there is way to tell midPoint that we want to completely ignore an item:
...
The simplest case is an conditional assignment of a role.
Let’s supposed that we want to assign a Hatter role to everybody that has provided a hat size in user profile.
...
Once is the condition evaluated for an object before a change is applied (old object).
The the condition is evaluated for an object after the change is applied (new object).
...
We are almost there.
The final part of this puzzle is an object template mapping that automatically assign the roles according to job code.
Naive solution would be to create one mapping for each job code.
But we do not want that.
We want something smarter.
We want a single mapping that can work for all those roles.
Such a mapping needs to dynamically look up the role when it is evaluated.
...
Therefore when the expression is processed for Bob then the final filter looks like this:
...
The filter can bu much more complex and it can have several expressions.
...
It can also create the target on demand in case that the target is not found.
It is quite powerful.
But what is perhaps most important is that `assignmentTargetSearch` expression implements the use cases that are common in almost every IDM deployment.
...
However, that functionality is not finished.
If you are interested in this functionality then midPoint platform subscription is a way to get it fully implemented.
...
One such case is a problem of finding a unique identifier.
...
Maybe `aanderson` would a good fit?
And this can indeed work quite well.
Until Albert Anderson is hired.
Then the we need to get creative.
Obviously, `alanderson` will not work here.
What about `alianderson` and `albanderson`?
Oh no, we have this ancient system that allows only ten characters in the username and those are too long.
And we would need to change Alice’s username.
...
Result of this mapping will be `aanderson1`. Which is an unique username.
Therefore iteration stops there and normal processing continues.
In case that even `aanderson1` is not unique, the iteration continues and `aanderson2`, `aanderson3` and other variants are tried until a unique username is found or until iteration limit is reached.
...
Iteration tokes are strings that are created from iteration number.
...
Which one of those is number one and which is number two anyway?
Let’s skip `aanderson1` altogether and let’s use `aanderson2` for the first iteration.
...
It is one thing to have username `aanderson2`. That username is used to log into the system.
But otherwise is it not very visible.
But an e-mail address is visible.
...
But what it is supposed to do with that.
...
But the mapping cannot do that.
...
The code above does not do much in case that e-mail address is unique.
It returns `true`, the iteration is accepted an everything goes as usual.
But in case that the e-mail address is not unique the code returns `false`.
...
Finding identifier values and checking for uniqueness is a messy stuff.
And it is not entirely reliable.
There is some delay between the time when uniqueness is checked and the time that the record is actually written into database.
Therefore strange things can happen.
Duplicate identifiers may be generated or attempt to create a user may end up with an error.
...
But it is feasible.
In case that you are interested midPoint platform subscription is the best approach for you.

When it comes to human-friendly identifiers there is yet another trouble.
People tend to change their minds.
And then they have all kinds of crazy ideas such as the urge to get married.
The result is that the names of people change.
And in fact they change surprisingly often.
...
But there is drawback to this approach.
Let’s suppose that Carol Cooper had username `ccooper2`. She got married and now her name is Carol Cunningham.
Even though there is no `ccunningham` in the system, her generated user name will be `ccunningham2`. The iteration token is remembered.
...
Once we have an owner of an account we can execute inbound mappings.
...
But this picture is good enough for now.
...
Those files represent the final configuration, the expected state at the end of this chapter.
...
Therefore let’s add iteration tokens into the mix.
Like this:
...
But it is still not a common practice to use national characters in usernames, e-mail addresses and so on.
...
If the iterative approach cannot find a match in several steps then perhaps iteration is not a good method anyway.
...
And it is much better to get an error after a couple a seconds than to wait forever.
...
Therefore go ahead and import the HR resource, import object template, set the object template in the configuration and do not forget to replace the HR CSV file.
If you did experiments with previous configuration it can be helpful to clean up midPoint by using the “delete all identities” process.
When everything is set up then you can try to manually import a single account from the HR resource by using the “import” button on the page where you can list resource accounts.
...
Do not forget to switch from repository to resource view by clicking on the _Resource_ button on the top-left side of the page.
There is no synchronization task running therefore MidPoint haven’t see Arnold’s account yet.
...
This mapping should be quite understandable now.
...
A clever reader can surely discover a could of obvious issues.
...
But midPoint does not consider e-mail address to be an identifier, therefore it does not check for its uniqueness.
Also, there is only one iteration token that is reused for all the expression in all object template mappings.
...
But it is quite likely that a completely custom code will be needed for a more complex cases.
...
But there is another way that is much more popular: autoassignment in object template mapping.
...
As simple as that.
Put the construction in the role and the reconcile HR resource or just recompute the users.
LDAP accounts will be created.

But we want something that is a bit more fancy.
Salespeople are a bit sensitive when it comes to their professional image.
Therefore they insist to have proper titles set up in company directory.
Not a problem.
...
Absolutely not.
We do not want to have too many roles as every roles is a maintenance burden.
...
This is the only roles that gives access to the CRM system for a sales manager.
...
This is nice little identity management deployment.
But there is still a lot of things to do here.
Maybe we want to set up a formalized organizational structure.
Maybe we need delegated administration.
Maybe we have several object types and we want to set up archetypes for them.
...
Now it is the time for experiments.
You will surely do a lot of things that are suboptimal or even outright wrong.
But never mind.
This is part of the learning process.
If you get to the dead end just scrap everything and start over.
