...
We can populate the organizational units with data.
...
When mis-configured, the synchronization often does nothing.
There is no error or any other obvious indication of an error.
Logging is your best friend in this case.
Enable logging of _synchronization service_ (`com.evolveum.midpoint.model.impl.sync`) at debug level.
MidPoint will log reasonable amount of information that is very likely to lead you to the solution.
...
Yet, even such a _stub_ object is sufficient to create organizational hierarchy.
...
TODO: provision orgstruct to LDAP
