...
Eric opens the resource detail page in the midPoint GUI, clicks on _Accounts_ tab and then on the _Resource_ button (we'll explain that later).
...
// TODO:4.4 update section names
For now, we only care about first two sections.
The _Basic_ section shows user properties as midPoint knows them.
