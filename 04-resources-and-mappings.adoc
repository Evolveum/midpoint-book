...
Identity resources (or just _resources_ for short) are typically target systems where midPoint manages accounts.
Moreover, source systems such as HR databases are also considered to be resources.
...
The list of discovered connectors can be seen in midPoint user interface in menu:Configuration[Repository objects > All objects] and select menu:Connector[] in the type field located at the top of the screen.
...
ConnId result filtering is a legacy mechanism and most connectors do not need that anymore.
