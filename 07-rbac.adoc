...
And then there are scenarios when we actually want reality to be different from policy for a given period of time.
...
When such assignment is added to a user, there is suddenly a discrepancy between reality and policy.
...
However, there is no such account.
Therefore, MidPoint aligns reality and policy by creating that account.
...
Later on you may add new application to your system, and you probably want all the users to have account there.
...
But how do we give this role to users?
...
But it is sufficient for the vast majority of cases.
...
MidPoint roles are very useful kind of animal, because they are used for almost everything in midPoint.
...
NOTE: Surprisingly, role universality is quite a unique concept in the IDM field.
...
Is role-based access control doomed to failure?
Certainly, there are people that proclaim that role-based approach is useless, suggesting many innovative approaches that are usually even more useless in practice.
Role-based access control is not dead, not even close.
It is just much harder than it may seem.
...
This _bottom-up_ approach may not be perfect for business analytics puritans, but it is a practical approach, favored by almost all practical organizations.

However, it is not practical to start with roles that have business meaning.
I.e., it is not practical to start with `Clerk` role, as we do not yet know what privileges should be in that role, as we do not know what privileges Harry has, and we may not even know that Harry is a clerk.
There are too many missing pieces to start solving the business puzzle yet.

We start by creating _application roles_.
Those roles deal with access to a single application.
...
The usual start is to create an application role for every _group_ in Active Directory or a similar central directory system.
Groups are good starting material.
They tend to be the right granularity, not too broad or too narrow.
...
An application role will grant account in Active Directory, as well as membership in one specific group.
MidPoint _entitlements_ mechanism is an ideal tool to do this.

.Entitlements and synchronization of application roles
NOTE: Indeed, midPoint _entitlements_ mechanism is an ideal tool for building application roles.
...
There are some information about _generic synchronization_ in the Organizational Structure chapter.

Strictly speaking, application roles are not meant to be assigned directly to users.
...
We have to accept that application roles are assigned directly to users, for now.
Although we are not strictly doing any serious role-based access control yet, we can already do some role-based magic even at this point.
...
Owners can be assigned to application roles, keeping an information who is responsible for each particular group.
...
Even though application roles are not perfect, they are practical.
We have to admit that.

Once we have the application roles, we can start combining them into roles that have a business meaning.
...
We may need to have a look at the roles of Violet Vickers who is also a clerk in the same organization unit and compute an intersection of Harry's and Violet's application roles.
...
It is very hard to get this exactly right.
...
This smells like huge spreadsheets that get outdated even sooner than the analysis is done.
...
Yet, some roles may be assigned to users in early stages of the deploment when business roles are not fully formed yet.
...
// TODO: mention archetypes ... reference to the unwritten chapter?
...
Overall, role engineering an art rather than a science.
...
The fact is that repetition is daily bread in almost all IDM deployments.
...
However, in this case, we want to apply meta-role to a role.
...
Therefore, the people that maintain dogmatic beliefs about IAM mechanisms should skip this section as well.
...
There many variants of RBAC and there are other access control models that are based on a completely different paradigm.
...
It would be a marvel if an IDM system could automatically determine the privileges of every person simply based on the organizational unit, work responsibilities or any other attributes of that person.
...
There is no efficient feedback loop that would force corrections in HR data - until the IDM system is deployed, that is.
It takes years or decades for a typical company to deploy an IDM system.
...
Identity system should not fail when the input data are wrong.

The IDM deployment should not be delayed because of wrong input data.
That would be like refusing to use your reading glasses because the text you are reading is misspelled.
IDM systems are essential tools that assist you in the process of data clean-up.
The IDM system should be deployed using whatever data are available, and it should be used to manage data quality on day-to-day basis.
It is naive to think that once the data are cleaned-up they will stay clean.
The processes that crated data errors are still there, therefore data errors will appear all the time.
...
This is not limited to IDM deployment project.
...
There are many things that need to be done manually.
Such decisions are made almost on day-to-day basis.
For ABAC, PBAC and similar systems this would mean that a policy needs to be updated on a day-to-day basis.
However, these systems are not designed for such flexibility.

Then there is another big problem: the data are incomplete.
...
The usual solution to this problem is that the user requests the privileges that are needed to do a job.
Such request is then routed through appropriate approval process.
That _request_ is a big problem for ABAC and PBAC.
What should the user request to get the privileges?
...
Can we somehow create a catalog of the things that a user can request?
...
It is quite easy to create a role catalog.
...
While ABAC/PBAC policy may be easy to set up, it is quite difficult to analyze and maintain.
...
However, it is all quite easy in RBAC.
Policies are encapsulated into roles.
...
The fact that ABAC/PBAC fails in complex practical IDM deployments does not mean that RBAC is ideal.
Quite the contrary.
...
And there can be any complex algorithm in the expression, even a complete ABAC/PBAC policy.
At least in theory.
...
Due to a momentary lapse of imagination we dubbed it _Advanced Hybrid RBAC_.
