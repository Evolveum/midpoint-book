...
It is quite easy to import all HR records into an empty midPoint: set up inbound mappings, start import task, wait a bit, and all is done.
...
Import may not be entirely trivial either, for example when we try to import data from an additional data source into a running midPoint deployment.
...
It works like this: whenever midPoint discovers new _account_ it will try to link that account to an existing _user_.
MidPoint looks for an appropriate _user_ to represent newly-discovered _account_.
It does so by matching selected _account_ attributes to appropriate user _properties_.
Two things are needed for correlation to work: _correlation rule_ and corresponding _inbound mapping_.

_Correlation rule_, also known as _correlator_, specifies which properties are used for correlation, and how they are used.
In its simplest form it simply points to a single user property:
...
_Correlator_ defined above specifies that the `personalNumber` property should be used to correlate _accounts_ and _users_.
If a newly-discovered account has a value that matches the `personalNumber` of a user, it is assumed that such user is an _owner_ of that account.
Such account and user will be _linked_.
...
This is what midPoint normally does when it maps account data to user data.
...
// TODO: this can be simplified <correlator/> in attribute definition
// TODO: NOTE: this is not used in examples (yet) as we have two inbound mappings for empno, it is a common practice, look at later chapters

// TODO: correlation-only mapping for target resources; reversible mappings in the future

// TODO: mention correlation expression as pre-4.8 mechanism

// TODO: 4.8

// TODO: correlation is a database search, it is important for the correlation property to be searchable
The correlation expression must be TODO TODO a search filter because that is the only efficient way how to find single user in a large set of other users.
We cannot scan the users one-by-one.
We need to utilize the search power of the database for this.

// TODO: smarter correlations, e.g. approximate with human interactions.
...
If we happen to find an account which should be linked but is not (situation: unlinked), then link it (action: `link`).
...
Even though this method would be perfectly acceptable for a company of this size, we are still going set up a live synchronization task.
