...
We will describe archetrypes later.
...
In this case the activity specifies that this is a synchronization task that imports accounts from the resource.
...
However, the task will not be executed unless you explicitly tell midPoint to do so.
...
There may be good old configuration problems, that are usually easy to fix.
There may be data compatibility problems, such as presence of non-ASCII national characters where they are not expected.
Worst of all, source data may be of poor quality, there may be inconsistencies, typos, the data may be out of date, overall not reflecting the reality very well.
...
And this is indeed the case in many of cases.
The CSV file is usually produced as an automatic scheduled export of HR system data, usually running every night.
...
For that reason, midPoint has both the cause and the effects correlated together in a single operation.
Therefore it is much easier to figure out what is going on.
Having the cause and effect connected in one operation means that it can also be neatly recorded in the audit trail.
...
This is indeed very useful configuration.
...
There is the ancient "Retro change log", going back to Netscape/iPlanet LDAP servers, that is still used today.
...
// TODO: Move up to mapping section?

// Note: This section was NOT reviewed for 4.4 changes
