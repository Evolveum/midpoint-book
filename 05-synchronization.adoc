...
// TODO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! TODO
// TODO: This section needs to be re-structured for the next review of the book
// See TODOs inside the text
// TODO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! TODO
...
That is where the synchronization mechanisms come in.
...
However, there are also less obvious cases, such as ordinary provisioning operation when midPoint needs to create a new account for a user.
...
Individual mechanisms differ in a way data inconsistency is _discovered_: livesync will actively look for new changes, reconciliation will compare the data one-by-one and opportunistic synchronization will discover inconsistency by pure chance.
Yet, all the mechanism _react_ to inconsistency in the same way.
...
Yet, there is just one big synchronization mechanism overall.
...
It contains only data about employees, and it has only partial information about them.
...
Yet, there often are pieces of authoritative information in here.
For example an algorithm to generate a username may be based on the usernames that are already used in the Active Directory.
...
That is it.
Think about the same mappings that were used in previous chapter, just flip the direction.
...
Password is usually write-only value.
...
In addition to that, `generate` expression evaluator will take user password policy into consideration.
...
Therefore, `generate` expression will look for password policy, and generate a random password that just matches requirements for password length and complexity.
...
We only want to set a random password in case that it is an initial password, the first password ever.
That is exactly what a weak mapping does: it sets new value only if the target does not have any existing value.
...
Import is usually not trivial either, for example in cases when we try to import data from an additional data source into a running midPoint deployment.
...
Correlation expression is a method to connect newly-discovered accounts and existing users.
...
The easiest form of the correlation expression is to search users using an identifier:

// TODO: 4.8: change to Axiom query
...
We cannot scan the users one-by-one.
...
On the other hand, this may be a reconciliation with a target resource, and we have just found an illegal account.
We probably want to disable such account in this case.
...
The situation reflects whether this account is already linked to the user, whether we know the candidate owner, whether we cannot determine the owner and so on.
...
The reaction may be quite clear for some situations (e.g. `unlinked`), but there is a lot of variability for other situations (e.g. `unmatched`).
...
The only think that deserves special explanation is the reaction to `linked` situation.
...
However, there is no reaction for `linked` situation.
...
E.g. inbound mappings are skipped in case that the user is about to be deleted, the mappings are also skipped if the reaction does not include "synchronization" and so on.
...
Reconciliation is quite a heavyweight task.
...
This task is usually not scheduled, it is almost always executed manually.
...
The tasks are necessary to initiate the synchronization.
...
Desprite that, practical deployments need at least one synchronization task to work properly.
...
The tasks are quite rich, and they may be a bit tricky to handle.
...
Of course, there are some special functions that only the tasks have (such as suspend and resume).
...
They are usually created by using special-purpose menu:Defined tasks[] tab in resource detail pages.
...
Even though this method would be perfectly acceptable for the company of this size, we are still goging set up a live synchronization task.
...
It simply specifies that the administrative status should be mapped in the inbound direction.
And that is it.
...
Therefore we are going to generate the passwords.
For that we are going to use the weak mapping with generate expression that was explained above.
...
Then there is an assignment of `Import task` archetype.
...
As import is not a recurring task, it will not _keep_ the data synchronized.
Import tasks are not designed to do so.
Fortunatelly, there are other tasks that are designed for continuous synchronization.
...
This is what makes this task a reconciliation task.
...
It is very reliable.
It is often used to fix data problems, apply new policies, look for missing accounts, illegal accounts and so on.
...
However, listing all the accounts often may put unacceptable load on the resources.
...
Or rather _almost_ real-time synchronization.
...
Setting up synchronization flavors is just a matter of setting up the tasks.
...
This is usually one of the first steps in an IDM project.
...
These problems are the most difficult to correct, as the right way to correct them is to modify the data at its source.
That takes time, meetings, mail messages, management decisions, processes, excuses, delays and a lot of patience.
...
But that is not always possible (in fact it is almost never a feasible option for a quick fix).
...
Therefore, there are no specialized propagation or provisioning tasks that you might know from older IDM systems.
...
Some IDM systems decouple the cause from the effect.
...
It is a very useful behavior during deployment of a new IDM system.
...
The strategy is configured on connector level, and the details should be, theoretically, hidden inside the connector.
...
Yet, we live in a practical world, and there are many details that leak through the _synchronization_ abstraction.
...
And then there is a catch-all synchronization mechanism that looks for recent changes based on `modifyTimestamp` attribute.
...
However, not all of them can delete accounts.
There are systems that keep the accounts forever, the accounts can be only permanently disabled.
Yet another systems cannot enable or disable accounts.
...
Such connectors are very useful, they are used in almost every midPoint deployment.
However, there is no standardized way to disable an account in database table or a CSV file.
...
There is way to tell midPoint which attribute and what values are used to represent account activation status.
...
However, LDAP connector is no ordinary simple connector.
...
That is the reason that the _Repository_ view is empty.
...
MidPoint was not told to do anything with the accounts.
...
Even though the `uid` value is obviously wrong, midPoint have linked the account correctly to the user (`Carol Cooper`).
...
Even if midPoint user does not provide any value for attribute `cn`, we can still determine that value by using the expression.
...
However, there are much better methods how to handle this.
...
Reconciliation does not only compare the accounts, it is actively fixing the inconsistencies.
...
Therefore, reconciliation is usually used in a form of a reconciliation task.
...
If a user is changed, then such change may propagate to other accounts on other resources, usually by the means of outbound mappings.
MidPoint does not like procrastination, and therefore it will try to apply all the changes immediately.
It means that reconciliation of one account may cause changes to other accounts.
...
To the untrained eye, _recompute_ does looks almost exactly the same as _reconciliation_.
However, there are subtle differences.
Firstly, recompute will not force the fetch of account data.
When _recomputing_, the account attributes will be fetched from the resource, unless midPoint inevitably needs them for the computation.
This usually happens if `weak` or `strong` mappings are used.
However, if there are `normal` mappings only, then recompute may skip reading account data.
...
Therefore, recompute task will iterate over midPoint _users_.
...
It determines owner of each account, compare the attributes and corrects them.
As this process iterates over real accounts on a resource, it can easily detect new accounts.
When the main stage is completed, then the next phase looks at account shadows stored in midPoint.
...
For example, use recompute after change of object templates or role definitions to apply the changes to all users.
...
That is one of the reasons why we need to explicitly specify sources of the mapping.
...
Or rather partially right at the very least.
...
Identity management solution _is_ in fact a distributed system, managing data in many independent applications and databases.
...
That is why relativistic approach is so useful.
...
For those rare cases where relativistic computation can fluctuate there is always reconciliation as a last resort.
...
An example of live synchronization task was provided in the HR feed section above.
...
There are (hopefuly quite rare) cases when resource and midPoint token get out of alignment.
...
// TODO TODO TODO review progress HERE
...
* We will see what is the real _quality_ of the data.
