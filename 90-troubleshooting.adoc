...
Several key people that took part in midPoint design were involved in IDM project since early 2000s.
...
All those systems were closed, their vendors jealously guarding the all the secrets of inner workings of those systems.
...
But even such a drastic actions usually did not help much.
...
Senior engineers would probably think that this is a silly question and that the answer is obvious.
But it is not.
...
Are there any information about the operation progress when log level is set to DEBUG?
...
If the answer is "yes" then then problem is probably in the connector.
...
|`UNKNOWN`
|Status of the operation is not know.
...
Logging provides information about the important things that happen in the system.
But it also goes very deep and it can provide very fine details about midPoint operations.
Logging is universal and very powerful.
It is the best hope to find the cause even for the trickiest of problems.

As for every other tool, the most important thing is to know how to use it properly.
...
But it is not enough in case that you are chasing a configuration.
In that case the logging system needs to be reconfigured to log more information.
But beware, if logging system is set to its full power you will get a huge stream of data that is likely to completely overwhelm you.
The important information will surely be there.
But they will be lost in the flood of other data.
...
Almost all of log messages from midPoint will be recorded here.
...
However, it is an useful place to look while experiencing startup issues.
...
It requires some instinct and experience.
...
Each level has a precise definition of the amount of details that it provides.
...
The first think to keep in mind is that midPoint is composed from subsystems and components.
...
It is always an advantage to see all the details of an operation that the user interface tries to initiate.
...
Therefore it can be used to get an overview of midPoint operations in the past.
...
MidPoint deployment usually record audit trails in the database table.
...
Clockwork also invoked ChangeExecutor to carry out the changes.
...
Therefore clockwork and projector work in _waves_.
...
MidPoint provides quite a lot diagnostic data for each step and each sub-step of each step.
...
There information that you are looking for will be almost certainly there, but it will be lost among all the innocent-looking data.
...
. In case that you suspect provisioning issues but you are not sure it may be helpful to have a detailed look at ChangeExecutor.
...
There are almost uncountable number of combinations, configurations, network conditions and other circumstances where things may go wrong.
...
Therefore it is very hard to find good names for them.
...
Mappings and expression often contain custom scripting code.
This means that midPoint is very flexible and can satisfy diverse requirements.
But it also means that mappings and expressions are often a source of problems.
...
MidPoint contains a code that can be used to trace execution of mappings and expressions on a very detailed level.
...
For example there are found update operations:
...
Therefore look at this log should be the first thing to do when there are problems with LDAP and AD connectors.
...
TIP: *Object security constraints and the user interface.* +
...
It is nearly impossible to test midPoint for all the conceivable configuration and use cases.
Whenever you deploy midPoint there is a change that some part of your configuration or usage patterns are unique.
...
Therefore all contributions including the bug fixes has to be reviewed.
...
However, typical fix times will be counted in days, weeks or in very rare and complicated cases in months.
...
There please make sure you use those facilities.
...
Try to figure out what is the minimal configuration that is needed to reproduce the problem.
...
This is also the only bugreporting method available for community issues.
...
Please note that security issues that reveal a potential security vulnerability should not be reported by using the tracker.
...
Which means that problem is in midPoint, somewhere between model and the connector.
